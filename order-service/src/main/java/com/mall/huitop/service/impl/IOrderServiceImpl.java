...
import com.mall.huitop.fegin.CouponOpenFeginService;
import com.mall.huitop.fegin.GoodsOpenFeginService;
import com.mall.huitop.fegin.UserOpenFeginService;
...
public class IOrderServiceImpl implements IOrderService {
    ...
    @Autowired
    private GoodsOpenFeginService goodsOpenFeginService;

    @Autowired
    private UserOpenFeginService userOpenFeginService;

    @Autowired
    private CouponOpenFeginService couponOpenFeginService;
    ...
    public Result confirmOrder(TradeOrder order) {
        ...
        try {
            ...
            return Result.ofFail(ShopCode.SHOP_FAIL.getCode(),ShopCode.SHOP_FAIL.getMessage());
        }
    }
    ...
    private void reduceMoneyPaid(TradeOrder order) {
        if(order.getMoneyPaid()!=null && order.getMoneyPaid().compareTo(BigDecimal.ZERO)==1){
            ...
            Result result = userOpenFeginService.updateMoneyPaid(userMoneyLog);
            if(result.isSuccess() == (ShopCode.SHOP_FAIL.getSuccess())){
    ...
    private void updateCouponStatus(TradeOrder order) {
        if(order.getCouponId() != null){
            TradeCoupon coupon = couponOpenFeginService.findOneByCouponId(Long.valueOf(order.getCouponId()));
            coupon.setOrderId(order.getOrderId());
            coupon.setIsUsed(ShopCode.SHOP_COUPON_ISUSED.getCode());
            coupon.setUsedTime(new Date());

            //更新优惠券状态
            Result result =  couponOpenFeginService.updateCouponStatus(coupon);
            if(result.isSuccess() == (ShopCode.SHOP_FAIL.getSuccess())){
    ...
    private void reduceGoodsNum(TradeOrder order) {
        ...
        Result result = goodsOpenFeginService.reduceGoodsNum(goodsNumberLog);
        if(result.isSuccess() == (ShopCode.SHOP_FAIL.getSuccess())){
    ...
    private Long savePreOrder(TradeOrder order) {
        ...
        if(moneyPaid != null){
            ...
            //余额大于0
            if(r == 1){
                TradeUser user = userOpenFeginService.findOneByUserId(Long.valueOf(order.getUserId()));
                //余额是非法值
        ...
        if(couponId!=null){
            TradeCoupon coupon = couponOpenFeginService.findOneByCouponId(Long.valueOf(couponId));
            //6.1 判断优惠券是否存在
            if(coupon == null){
    ...
    private void checkOrder(TradeOrder order) {
        ...
        //2.校验订单中的商品是否存在
        TradeGoods goods = goodsOpenFeginService.findOne(Long.valueOf(order.getGoodsId()));
        if (goods == null) {
